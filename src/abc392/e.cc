...
int main() {
  ...
  set<int> groups;
  rep(i, n) { groups.insert(uf.find_root(i)); }
  ...
  int step = groups.size() - 1;

  for (auto cable_i : cables) {
    int group_a = uf.find_root(av[cable_i]);
    while (true) {
      int group_x = *groups.begin();
      groups.erase(group_x);
      if (!uf.is_same_group(group_a, group_x)) {
        cout << cable_i + 1 << " " << av[cable_i] + 1 << " " << group_x + 1
             << endl;
        uf.unite(group_a, group_x);
        if (--step == 0) {
          return 0;
        }
        break;
