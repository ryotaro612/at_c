...
int main() {
  ...
  cin >> n >> k;
  Mod<MOD> p = 1;
  Mod<MOD> n2 = Mod<MOD>(n).pow(2);
  Mod<MOD> sub = (Mod<MOD>(1) + (Mod<MOD>(n - 1ll).pow(2))) / n2;
  Mod<MOD> sub2 = Mod<MOD>(2) / n2;
  rep(i, k) {
    Mod<MOD> next_p = p * sub + (Mod<MOD>(1ll) - p) * sub2;
    p = next_p;
  }
  Mod<MOD> res = Mod<MOD>(1) * p;
  Mod<MOD> temp =
      (Mod<MOD>(1) / Mod<MOD>(2)) * Mod<MOD>(n) * Mod<MOD>(n + 1ll) -
      Mod<MOD>(1);
  res += (Mod<MOD>(1) - p) * temp / Mod<MOD>(n - 1ll);
  cout << res << endl;
  return 0;
}
