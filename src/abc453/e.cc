...
int main() {
  ...
  Bit<int> bit_left(n), bit_right(n);
  rep(i, n) {
    int l, r;
    cin >> l >> r;
    right.insert({l, r});
  ...
  Mod<MOD> res = 0;
  ModCombi<MOD> combi(n + 1);

  for (int i = 1; i <= n - i; i++) {
    int j = n - i;
    dbg(i, j);
    while (right.size() && right.begin()->first <= i) {
      auto p = *right.begin();
      right.erase(right.begin());
      bit_right.add(p.second, -1);

      left.insert(p);
      bit_left.add(p.second, 1);
    }
    // jがLを満たしているかのチェックはあるか？
    if (right.size() && right.rbegin()->first > j) {
      dbg("continue");
      continue;
    }

    dbg(left, right);
    // rep(x, n) { dbg(x, bit_left.get(x), bit_right.get(x)); }
    // Rの判定
    if (0 < bit_left.sum(i) || 0 < bit_right.sum(j)) {
      dbg("continue");
      continue;
    }
    int must_left = bit_left.sum(j);
    int must_right = right.size();
    int any = bit_left.sum_range(j, n);
    dbg(must_left, must_right, any);
    // assert(j - must_right == any - must_left);
    if (i < must_left || j < must_right) {
      dbg("continue");
      continue;
    }
    dbg("combi", any, i - must_left);
    Mod<MOD> temp = Mod<MOD>(combi.c(any, i - must_left));
    dbg(temp);
    if (i != j) {
      res += temp * Mod<MOD>(2);
    } else {
